#!/usr/bin/env python3
"""
Restore the complete Shopify integration GUI and fix image upload issues
"""

def restore_full_shopify_config():
    """Restore the complete, feature-rich Shopify config with image fix"""
...
class ShopifyClient:
    ...
    def _validate_image_url(self, url):
        """Validate and fix image URLs for Shopify"""
        try:
            ...
            if any(ext in url_lower for ext in valid_extensions) or 'wilo.com' in url_lower:
                # Test if URL is accessible
                try:
                    response = requests.head(url, timeout=5, allow_redirects=True)
                    if response.status_code == 200:
                        content_type = response.headers.get('content-type', '').lower()
                        if 'image/' in content_type or any(ext in url_lower for ext in valid_extensions):
                            return url
                except:
                    pass
    ...
    def create_product(self, product_data):
        """Create a single product in Shopify - WITH IMAGE VALIDATION"""
        try:
            print("=" * 80)
            ...
            # FIXED IMAGE HANDLING - Validate all URLs first
            valid_images = []
            card_image = product_data.get('card_image_url', '')
            product_images = product_data.get('product_images', [])
            
            print(f"🔍 Processing images: card={bool(card_image)}, product_images={len(product_images)}")
            ...
            # Add valid images to product (ONLY if we have valid ones)
            if valid_images:
...
class ShopifyConfig(ttk.LabelFrame):
    ...
    def _create_widgets(self):
        ...
        # Image validation option
        self.validate_images_var = tk.BooleanVar(value=True)
        ttk.Checkbutton(
            options_frame,
            text="Validate image URLs (prevents upload errors)",
            variable=self.validate_images_var
        ).pack(anchor='w')
    ...
    def _upload_worker(self, products):
        """Worker thread for uploading products - WITH FIXED IMAGE HANDLING"""
        try:
...
'''
    ...
    with open('gui/widgets/shopify_config.py', 'w') as f:
        f.write(full_shopify_config)
    
    print("✅ Restored complete Shopify GUI with image validation fix")

def main():
    print("🔧 RESTORING COMPLETE SHOPIFY INTEGRATION GUI")
    print("=" * 55)
    
    print("Restoring the full-featured Shopify integration interface with:")
    print("• Complete connection settings panel")
    print("• Upload progress tracking")  
    print("• Upload options (drafts, skip existing)")
    print("• Results display with detailed logs")
    print("• Image URL validation and testing")
    print("• Export/import functionality")
    print("• Real content upload with debug logging")
    
    try:
        restore_full_shopify_config()
        ...
        ast.parse(content)
        print("\n✅ SHOPIFY GUI FULLY RESTORED!")
        print("✅ Python syntax is valid")
        
        print("\n🎯 Key features restored:")
        print("• Connection testing with proper status display")
        print("• Progress bar with real-time updates")
        print("• Upload options (draft mode, skip existing)")
        print("• Image validation to prevent 422 errors")
        print("• Detailed results logging")
        print("• Export to JSON functionality")
        print("• Upload from file capability")
        
        print("\n🔧 Image upload fix:")
        print("• Validates all image URLs before sending to Shopify")
        print("• Tests image accessibility with HTTP HEAD requests")
        print("• Skips invalid images instead of failing entire product")
        print("• Shows detailed logs of which images are valid/invalid")
        
        print("\n🚀 Test the complete workflow:")
        print("1. python main.py")
        print("2. Extract products (already working perfectly)")
        print("3. Go to Shopify Integration tab")
        print("4. Test connection")
        print("5. Upload scraped products")
        print("6. Watch for detailed debug logs and image validation")
        
        print("\n📊 Expected results:")
        print("✅ Products upload successfully with real content")
        print("✅ Valid images are included")
        print("⚠️ Invalid images are skipped with warnings")
        print("❌ No more 422 'Image URL is invalid' errors")
